...
- Ports are configured in the root `.env`. If 5432, 3000, or 5173 is taken on this machine, change `DB_PORT` (together with the port inside `DATABASE_URL`), `PORT` (together with `BETTER_AUTH_URL`), or `WEB_PORT` — don't kill other projects' containers.
